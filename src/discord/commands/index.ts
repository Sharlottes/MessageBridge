...
namespace CommandManager {
    ...
    export async function register(command: Command): Promise<boolean> {
        ...
        if(process.env.DEBUG) console.log(`[Command] register [ /${command.builder.name} ] to ${command.category} command.`);
        return true;
    }
    ...
    export async function refreshCommand(target: "global" | "guild", guild?: Guild): Promise<void> {
        ...
        // 명령어 제거
        await Promise.all((await Discord.client.rest.get(commandPath) as CommandInfo[]).map(command => 
            Discord.client.rest.delete(`${commandPath}/${command.id}`)
        ));

        // 명령어 재등록
        await Promise.all(commands.reduce((sequence, command) => 
            command.category == target 
                ? [...sequence, application.commands.create(
