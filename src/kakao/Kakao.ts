...
import { TextChannel, ButtonBuilder, ComponentType, ButtonStyle, ActionRowBuilder } from "discord.js";
...
namespace Kakao {
    ...
    export async function init(): Promise<void> {
        ...
        server.on('message', async (message) => {
            ...
            if (process.env.DEBUG) console.log(`[${message.room}] ${message.sender.name}: ${message.content}`);
        });
...
export function sessionList(msg: Message) {
  ...
  export function linkChannel(msg: Message, guildID: string, channelID: string) {
    ...
    if (channel instanceof TextChannel) {
      const exist = ChatLinkManager.chats.find(chat => chat.kakao == msg.room && chat.discord.id == channelID);
          ...
          .then(interaction => {
            switch (interaction.customId) {
              case 'accept': {
                ...
                const kakaolinks = ChatLinkManager.chats.filter(chat => chat.kakao == msg.room);
                if (kakaolinks.length > 1) msg.reply(`경고: ${msg.room}에서 다수의 연결 감지\n${kakaolinks.map(chat => `${chat.kakao} <-----> ${chat.discord.name}`).join('\n')}`).catch(console.log);
                const discordlinks = ChatLinkManager.chats.filter(chat => chat.discord.id == channelID);
  ...
  export function dislinkChannel(msg: Message, guildID: string, channelID: string) {
    ...
    const index = ChatLinkManager.chats.findIndex(link => link.discord.id == channel.id);
